...
static void fts_key_trigger_delta_read(void *chip_data)
{
	...
	if (s == NULL) {
		...
		TPD_INFO("0xE1 = %d, 0xE2 = %d, 0xE3 = %d, 0xE4 = %d, 0xE5 = %d, 0xE6 = %d, 0xE7 = %d, 0xE8 = %d\n",
			 ...
			 touch_i2c_read_byte(ts_data->client, 0xE2), touch_i2c_read_byte(ts_data->client,
					 0xE3), touch_i2c_read_byte(ts_data->client, 0xE4), \
			 touch_i2c_read_byte(ts_data->client, 0xE5), touch_i2c_read_byte(ts_data->client,
					 0xE6), touch_i2c_read_byte(ts_data->client, 0xE7), \
				...
				int max_num)
{
	...
	for (i = 0; i < max_num; i++) {
		...
		if (!ts_data->high_resolution_support) {
			...
			points[pointid].x = ((buf[2 + base] & 0x0F) << 10) + ((buf[3 + base] & 0xFF) <<
					    2)
					    + ((buf[6 + base] & 0xC0) >> 6);
			points[pointid].y = ((buf[4 + base] & 0x0F) << 10) + ((buf[5 + base] & 0xFF) <<
					    2)
					    + ((buf[6 + base] & 0x30) >> 4);
			points[pointid].touch_major = buf[7 + base];
			points[pointid].width_major = buf[7 + base];
...
static int ft3518_parse_dts(struct chip_data_ft3518 *ts_data,
