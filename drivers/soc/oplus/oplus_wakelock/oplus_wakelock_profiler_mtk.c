...
char *buf)
{
	...
	for (i = 0; (desc = all_modules[i]) != NULL && buf_offset < PAGE_SIZE; i++) {
		if(!strncmp(desc->module_name, "abort",
			strlen("abort"))||!strncmp(desc->module_name, "other", strlen("other"))||!strncmp(desc->module_name, "wakeup_sum", strlen("wakeup_sum"))) {
	...
	for (i = STATICS_NUMBER -1; i >= 0; i--) {
		if((max_wakelock_list[i].ws != NULL) && (max_wakelock_list[i].max_ws_name[0] != 0)) {
			max_ws_rate = ktime_compare(wall_delta,
									max_wakelock_list[i].max_ws_time) >= 0 ? ktime_to_ms(max_wakelock_list[i].max_ws_time)*100/ktime_to_ms(wall_delta) : 100;
			buf_offset += scnprintf(buf + buf_offset, 200, "%s\t%lld\t%d\n", max_wakelock_list[i].max_ws_name,
																			 ktime_to_ms(max_wakelock_list[i].max_ws_time),
																			 max_ws_rate);
		}
	}
	mutex_unlock(&max_wakelock_list_lock);
...
late_initcall(wakelock_profile_init);
