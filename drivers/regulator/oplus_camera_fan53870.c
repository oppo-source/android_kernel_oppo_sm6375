...
struct fan53870_pw_chip {
    ...
    unsigned int enable_count[FAN53870_LDO_NUM_MAX];
    struct mutex lock;
};
...
int fan53870_ldo1_set_voltage(int set_uV)
{
    ...
    if (set_uV == 1200000) {
        reg_value = 0x99;    /*1.232V*/
    } else {
        reg_value = 0x8F;    /*1.152V*/
    }
...
int fan53870_cam_ldo_set_voltage(int LDO_NUM, int set_mv)
{
    ...
    pr_info("fan53870_cam_ldo_set_voltage LDO_NUM:%d set_mv:%d\n", LDO_NUM, set_mv);
    mutex_lock(&pchip->lock);
    if (LDO_NUM > 0 && LDO_NUM < FAN53870_LDO_NUM_MAX) {
        ++pchip->enable_count[ldo_idx];
        pr_info("fan53870 LDO %d ref count %u", LDO_NUM, pchip->enable_count[ldo_idx]);
    }
    switch(LDO_NUM){
        case 1:
            reg_value = ((set_mv - 800)/8 + 99)&0xff;
        ...
        break;
        case 3:
            reg_value = ((set_mv - 1500)/8 + 16)&0xff;
...
int fan53870_cam_ldo_disable(int LDO_NUM)
{
    ...
    pr_info("fan53870_cam_ldo_disable LDO-%d ref count %u\n", LDO_NUM, pchip->enable_count[ldo_idx]);
    mutex_lock(&pchip->lock);
    if (LDO_NUM > 0 && LDO_NUM < FAN53870_LDO_NUM_MAX && pchip->enable_count[ldo_idx] > 0) {
        --pchip->enable_count[ldo_idx];

        if (pchip->enable_count[ldo_idx] == 0) {
            LDO_NUM = 1 << (LDO_NUM - 1);
