...
#define SY6970_HVDCP_DETECT_TO_DETACH_TIME 450
#define CPU_INTERNEL_TIME 1000000
#define DEFAULT_INPUT_CURRENT 500
static irqreturn_t sy6970_irq_handler(int irq, void *data)
{
	...
	chg_info("(%d,%d)\n",prev_pg,bq->power_good);
	if (bq->hvdcp_checked && bq->power_good && prev_pg) {
		bq->hvdcp_detect_time = cpu_clock(smp_processor_id()) / CPU_INTERNEL_TIME;
	}
	if (!bq->power_good && prev_pg && bq->hvdcp_detect_time > 0) {
	    bq->hvdcp_detach_time = cpu_clock(smp_processor_id()) / CPU_INTERNEL_TIME;
		chg_info("notify have get the detach time is %d the detect time is %d\n", bq->hvdcp_detach_time, bq->hvdcp_detect_time);
		if (bq->hvdcp_detach_time - bq->hvdcp_detect_time <= SY6970_HVDCP_DETECT_TO_DETACH_TIME) {
			sy6970_disable_hvdcp(bq);
			bq->sy6970_keep_vbus_5v = true;
			bq->is_force_aicl = false;
			bq->chg_cur = 0;
			bq->nonstand_retry_bc = false;
			bq->is_bc12_end = true;
			bq->aicr = DEFAULT_INPUT_CURRENT;
			bq->qc_to_9v_count = 0;
			bq->pre_current_ma = -1;
			...
			bq->hvdcp_detect_time = 0;
			bq->sy6970_keep_vbus_5v = false;
		}
	}
...
void vol_convert_work(struct work_struct *work)
{
	...
	if (!g_bq->pdqc_setup_5v) {
		if (oplus_sy6970_get_vbus() < SY6970_5V_THRES_MV) {
			...
			icharging = oplus_get_charger_current()/UNIT_TRANS_1000;
...
bool oplus_chg_check_qchv_condition(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < PULL_UP_9V_CHARGER_VOL
		&& chip->soc < COOL_DOWN_5V_UI_SOC_LEVEL && (chip->limits.tbatt_pdqc_to_9v_thr < 0
		|| chip->temperature <= chip->limits.tbatt_pdqc_to_9v_thr) && !chip->cool_down_force_5v && g_bq->qc_to_9v_count < BLACK_COOL_DOWN_COUNT) {
		return true;
	}
...
#define DEFAULT_CHG_VOL 3000;
#define KEEP_CHG_VOL 5000;

int oplus_sy6970_get_vbus(void)
{
	int chg_vol = DEFAULT_CHG_VOL;
	if(g_bq && g_bq->sy6970_keep_vbus_5v) {
		chg_vol = KEEP_CHG_VOL;
		return chg_vol;
	}
	if (g_bq) {
		chg_vol = sy6970_adc_read_vbus_volt(g_bq);
...
int oplus_sy6970_set_ichg(int cur)
{
	...
	if (chip->is_double_charger_support) {
		if (chip->slave_charger_enable || chip->em_mode ) {
			...
			ret = sy6970_set_chargecurrent(g_bq, main_uA/1000);
			if ( !ret ) {
			...
			if (chip->sub_chg_ops) {
				...
				if (chip->sub_chg_ops->charging_current_write_fast) {
					ret = chip->sub_chg_ops->charging_current_write_fast(slave_uA/1000);
					if (ret) {
			...
			ret = sy6970_set_chargecurrent(g_bq, uA/1000);
		}
	} else {
		ret = sy6970_set_chargecurrent(g_bq, uA/1000);
		if (ret) {
